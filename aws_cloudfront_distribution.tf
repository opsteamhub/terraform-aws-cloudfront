...
resource "aws_cloudfront_distribution" "distribution" {
  ...
  viewer_certificate {
    ...
    ssl_support_method             = length(compact([each.value["viewer_certificate"]["iam_certificate_id"], each.value["viewer_certificate"]["acm_certificate_arn"]])) > 0 ? each.value["viewer_certificate"]["ssl_support_method"] : null
  }
}