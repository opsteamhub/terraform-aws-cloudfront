    ...
    flatten(
      [
        ...
        [for z in [x["default_cache_behavior"]] :
          merge(z, { "origin_request_policy_id" : sha1(format("%s-default-%s", x["id"], z["target_origin_id"])) }) if length(compact([z["origin_request_policy"]["cookies_config"]["cookie_behavior"], z["origin_request_policy"]["headers_config"]["header_behavior"], z["origin_request_policy"]["query_strings_config"]["query_string_behavior"]])) > 0
        ]
      ]
