  ...
  cloudfront_distribution_config_normalized = [ for x in var.cloudfront_distribution_config:
    {
      ...
      default_cache_behavior = {
        ...
        function_association = can(x["default_cache_behavior"]["function_association"]) ? [ for y in x["default_cache_behavior"]["function_association"]:
          {
            event_type                    = try(y["event_type"], null)
            function_arn                  = try(y["function_arn"], null)
          }
        ] : []
        lambda_function_association = can(x["default_cache_behavior"]["lambda_function_association"]) ? [ for y in x["default_cache_behavior"]["lambda_function_association"]:
          {
            event_type                   = try(y["event_type"], null)
        ...
        target_origin_id               = try(x["default_cache_behavior"]["target_origin_id"], null)
        #trusted_key_groups             = 
        #trusted_signers                = 
      ...
      ordered_cache_behavior = [ for z in coalesce(x["ordered_cache_behavior"], []):
        {
          ...
          cache_policy_id                = try(x["cache_policy_id"], null)
          compress                       = try(x["compress"], null)
          default_ttl                    = try(x["default_ttl"], null)
          field_level_encryption_id      = try(x["field_level_encryption_id"], null)
          forwarded_values = {
            ...
            query_string_cache_key            = try(z["forwarded_values"]["query_string_cache_key"], null)
          }
          ...
          origin_request_policy_id       = try(z["origin_request_policy_id"], null)
          path_pattern                   = try(z["path_pattern"], null)
          #realtime_log_config_arn        =    
          #response_headers_policy_id     =               
          smooth_streaming               = try(z["smooth_streaming"], null)
          target_origin_id               = try(z["target_origin_id"], null)
          #trusted_key_groups             = 
          #trusted_signers                = 
